...
class ScLayer(nn.Module):
    def __init__(self, cfg, input_size):
        ...
        # self.fc = nn.Linear(c * h * w, 6)     # no-crop single grey scale
        self.fc = nn.Linear(c * h * w, 1)       # cropped grey scale images

    def forward(self, x):
        # x: (batch, n_crop, chan=1, H, W)
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            with torch.no_grad():
                ...
                x_sc = torch.randint(low=0, high=6, size=x_sc.shape)
